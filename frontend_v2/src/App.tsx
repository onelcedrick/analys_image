...
function Shell() {
  ...
  /* --------------------- F2 : transfert — seulement onglet Transfer Lab -- */
  useEffect(() => {
    if (tab !== "transfert") return;
    if (!target || !palette) return;
    if (!useServer && (!targetLab || !paletteLab)) return;
    let active = true;
    const timer = window.setTimeout(() => {
      setBusy(useServer ? "Transfert sur le serveur (peut prendre 1–3 min)…" : "Transport optimal en CIE Lab…");
      const run = async () => {
        const t0 = performance.now();
        try {
          const params = {
          ...
          if (useServer) {
            ...
            res = await new Promise<TransferResult>((resolve) =>
      ...
      run();
    }, 600); // debounce sliders
    return () => {
      active = false;
      window.clearTimeout(timer);
      setBusy(null);
    };
  }, [tab, target, palette, targetLab, paletteLab, trParams, useServer, notify]);

  /* --------------------- F4 : texture — seulement onglet Texture --------- */
  useEffect(() => {
    if (tab !== "texture") return;
    if (!target) return;
    let active = true;
    const timer = window.setTimeout(() => {
      setBusy(useServer ? "Texture sur le serveur (peut prendre 1–2 min)…" : "Histogramme conjoint + CLAHE/bilatéral…");
      const run = async () => {
        const t0 = performance.now();
        try {
          const params = { clip: txParams.clip, smooth: txParams.smooth, blend: txParams.blend / 100 };
  ...
  }, [tab, target, txParams, useServer, notify]);
  ...
  const loadFile = useCallback(
    (file: File, as: "target" | "palette") => {
      ...
      loadImage(url)
        .then(async (img) => {
          ...
          // Upload serveur d'abord — l'onglet Signal reste actif (pas de transfert auto)
          if (useServer) {
            try {
              setBusy("Envoi au serveur (Signal)…");
              id = await uploadToServer(data);
            } catch (e) {
              notify(`Upload serveur échoué — ${(e as Error).message}`);
            } finally {
              setBusy(null);
            }
          }
          const slot = { name: file.name, data, id };
          if (as === "target") {
            setTarget(slot);
            setTab("signal"); // rester sur Signal après upload
            setTrRes(null);
            setTxRes(null);
            setFxRes(null);
          } else {
          ...
          notify(`${as === "target" ? "Cible" : "Palette"} chargée — ${file.name}${id ? " · serveur" : ""}`);
        })
        .catch(() => notify("Fichier illisible (JPG/PNG attendus)"))
        .finally(() => {
          URL.revokeObjectURL(url);
    ...
    [addUploadedImage, notify, useServer]
  );

  const runForensicNow = () => {
    if (!target || busy) return;
    setBusy(useServer ? "DCT 8×8 sur le serveur (peut prendre 1–3 min)…" : "DCT 8×8 — analyse forensique…");
    const run = async () => {
      const t0 = performance.now();
      try {
        let res: ForensicResult;
